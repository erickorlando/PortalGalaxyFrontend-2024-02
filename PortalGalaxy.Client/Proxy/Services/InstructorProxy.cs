...
public class InstructorProxy : CrudRestHelperBase<InstructorDtoRequest, InstructorDtoResponse>, IInstructorProxy
{
    ...
    {
        var response =
            await HttpClient.GetFromJsonAsync<BaseResponseGeneric<ICollection<InstructorDtoResponse>>>(
                $"{BaseUrl}?filtro={filtro}&nroDocumento={nroDocumento}&categoriaId={categoriaId}");
